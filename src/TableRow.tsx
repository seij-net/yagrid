...
export const TableRow: React.FC<TableRowProps<any>> = (
    ...
    const cells = itemDefinitions.map(def => {
        ...
        return <TableCell key={ def.name } editing={ editing } render={ def.render } editor={ def.editor }
                          item={ item } itemPropertyName={ def.name } onEditItemChange={onEditItemChange} />
    })

    const editableActionsCell = actionsItemDisplay ? <td key="__TABLE_EDITABLE__">
        <TableItemActions item={ item } editingState={ editingState } actionItemList={ actionsItem } handleAction={ onActionItemDispatch }/>
    </td> : null
    return <tr>{ editableActionsCell }{ cells }</tr>
}
